...
BreakOut::BreakOut(int width, int height, int fps) : 
    ...
    _p1(static_cast<float>(width) / 5.0f, static_cast<float>(height) / 3.0f, 1),
    _p2(static_cast<float>(width) / 5.0f, 2.0f * static_cast<float>(height) / 3.0f, 1),
    _p3(4.0f * static_cast<float>(width) / 5.0f, static_cast<float>(height) / 3.0f, 1),
    _p4(4.0f * static_cast<float>(width) / 5.0f, 2.0f * static_cast<float>(height) / 3.0f, 1),
    _theta(0.0f) {
...
void BreakOut::render() {
  ...
  */
  ...
  float half_width  = static_cast<float>(_p1.x + _p2.x + _p3.x + _p4.x) / 4.0f;
  float half_height = static_cast<float>(_p1.y + _p2.y + _p3.y + _p4.y) / 4.0f;
  ...
  float p4x = _p4.x - half_width;
  float p4y = _p4.y - half_height;
  ...
  Vector3 p1(half_width + (p1x * cos_theta - p1y * sin_theta), half_height + (p1x * sin_theta + p1y * cos_theta), 1),
          ...
          p4(half_width + (p4x * cos_theta - p4y * sin_theta), half_height + (p4x * sin_theta + p4y * cos_theta), 1);
  ...
  screen.fillTriangle(v2, v3, v4, _texture.image_handle);
  screen.flush();
}
