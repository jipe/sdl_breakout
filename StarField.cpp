...
StarField::StarField(int width, int height, int number_of_stars)
  ...
  for (int i = 0; i < _number_of_stars; i++) {
    Star star;
    star.x = (rand() % (2*_width)) - _width;
    star.y = (rand() % (2*_height)) - _height;
    star.z = (_far - _near) * static_cast<float>(rand()) / static_cast<float>(RAND_MAX) + _near;
